// biome-ignore-all lint/performance/noAwaitInLoops: Readiness polling must be sequential and bounded by its deadline.
...
if (
	...
	try {
		...
		// biome-ignore lint/suspicious/noUnnecessaryConditions: runSmoke mutates success after runtime browser verification.
		if (!receipt.success) {
			process.exitCode = 1;
		}
	} catch (error) {
